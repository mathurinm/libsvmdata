...
class AbstractDataset(ABC):
    ...
    def _load_file_and_save_data(self, raw_dataset_path, ext_dataset_path):
        """
        In the derived class, this function is responsible of the transformation
        of the raw dataset file into two .npy/.npz files containing the feature
        matrix X and the response vector/matrix y. These files must be named
        <self.dataset_name>_X.<npz/npy> and <self.dataset_name>_y.<npz/npy>.
        This function is also responsible for removing the raw dataset file when
        needed.
        """
        pass
    ...
    def get_X_y(self, replace=False, verbose=False):
        ...
        .npz and/or .npy files, they are not re-downloaded, unless replace=True.
        """
